...
const collectCandidateTexts = (doc: Document): string[] => {
  ...
  if (!candidates.length) {
    const bodyText = sanitizeMultiline(
      (doc.body as HTMLElement | null)?.innerText || doc.body?.textContent || ''
    );
    if (bodyText) {
...
export const extractBossResumeProfileFromDocument = (doc: Document): Record<string, string> => {
  ...
  const profileSummary = queryFirstText(doc, [
    '#summary .info-text',
    '.advantage-text',
    '#summary',
  ]);
  const expectedJob = queryFirstText(doc, [
    '#purpose .expect-list-item',
    '#purpose .primary-info',
    '#purpose',
  ]);
