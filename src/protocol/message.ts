...
import type {
  ...
  MessageReadConstructorOptions,
  TechwolfChatProtocolPayload,
  TechwolfMessage,
  TechwolfMessageReadPayload
} from "./types";
...
const getRuntimeWindow = (): ChatRuntimeWindow => Tools.window as ChatRuntimeWindow;

const isChannelConnected = (channel?: ChatChannel): boolean => {
...
export class Message {
  ...
  constructor({ form_uid, to_uid, to_name, content, image }: MessageConstructorOptions) {
    const r = new Date().getTime();
    const d = r + 68256432452609;
    const data: TechwolfChatProtocolPayload = {
      messages: [
        {
          from: {
            uid: form_uid,
    ...
    this.msg = protobufType.encode(data as unknown as Record<string, unknown>).finish().slice();
    this.hex = [...this.msg].map((b) => b.toString(16).padStart(2, "0")).join("");
  }

  toArrayBuffer(): ArrayBuffer {
    return (this.msg.buffer as ArrayBuffer).slice(0, this.msg.byteLength);
  }
...
export class MessageRead {
  ...
  constructor({ userId, messageId }: MessageReadConstructorOptions) {
    const r = new Date().getTime();
    const data: TechwolfMessageReadPayload = {
      messageRead: [
        {
          messageId,
          readTime: r,
  ...
  send(): void {
    const runtimeWindow = getRuntimeWindow();
    if (runtimeWindow.ChatWebsocket) {
      runtimeWindow.ChatWebsocket.send(this);
      return;
    }
    if (runtimeWindow.ChatWebsocketImage) {
      runtimeWindow.ChatWebsocketImage.send(this);
    }
  }
}
