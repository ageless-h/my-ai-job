...
export class BossOption {
  ...
  static autoResumeSessionCount = 0;
  static autoReplyLastTs = 0;
...
`;
        ...
        getAutoReplySafetyConfig() {
          ...
          return {
            minReplyIntervalSec: Math.max(3, toNumberOr(preference.chatMinReplyIntervalSec, 12)),
            maxPerMinute: Math.max(1, toNumberOr(preference.chatMaxPerMinute, 6)),
            maxSessionReplies: Math.max(1, toNumberOr(preference.chatMaxSessionReplies, 75)),
            maxSessionResumes: Math.max(1, toNumberOr(preference.autoResumeMaxPerSession, 12)),
            timeWindowEnabled: preference.safetyTimeWindowE !== false,
            allowedStartHour: Math.min(23, Math.max(0, toNumberOr(preference.safetyStartHour, 8))),
            allowedEndHour: Math.min(23, Math.max(0, toNumberOr(preference.safetyEndHour, 22)))
          };
        }
        isInSafetyTimeWindow(startHour, endHour) {
        ...
        checkAutoReplySafety(action, reserve = false) {
          const safety = this.getAutoReplySafetyConfig();
          if (safety.timeWindowEnabled && !this.isInSafetyTimeWindow(safety.allowedStartHour, safety.allowedEndHour)) {
            return { ok: false, reason: `当前不在安全时段（${safety.allowedStartHour}:00-${safety.allowedEndHour}:00）` };
          }
          const verifyReason = Tools.getManualVerificationReason();
          if (verifyReason) {
            return { ok: false, reason: `检测到人工验证（${verifyReason}）` };
          }
          const now = Date.now();
          while (BossOption.autoReplyRecentTs.length > 0 && now - BossOption.autoReplyRecentTs[0] > 6e4) {
            BossOption.autoReplyRecentTs.shift();
          }
          if (BossOption.autoReplyRecentTs.length >= safety.maxPerMinute) {
            return { ok: false, reason: `自动回复频率过高（每分钟最多${safety.maxPerMinute}次）` };
          }
          if (BossOption.autoReplyLastTs > 0) {
            const elapsedSec = (now - BossOption.autoReplyLastTs) / 1e3;
            if (elapsedSec < safety.minReplyIntervalSec) {
              return { ok: false, reason: `自动回复间隔过短（至少${safety.minReplyIntervalSec}秒）` };
            }
          }
          if (BossOption.autoReplySessionCount >= safety.maxSessionReplies) {
            return { ok: false, reason: `自动回复达到会话上限（${safety.maxSessionReplies}）` };
          }
          if (action === "resume" && BossOption.autoResumeSessionCount >= safety.maxSessionResumes) {
            return { ok: false, reason: `自动发简历达到会话上限（${safety.maxSessionResumes}）` };
          }
          if (reserve) {
            this.markAutoReplySafety(action);
          }
        ...
        markAutoReplySafety(action) {
          const now = Date.now();
          BossOption.autoReplyRecentTs.push(now);
          BossOption.autoReplyLastTs = now;
          BossOption.autoReplySessionCount++;
          if (action === "resume") {
            BossOption.autoResumeSessionCount++;
          }
        }
        /**
         * 前置回复boss消息
        ...
        renderChatBubble(fromName, fromAvatar, question, answer) {
          const safeFromName = this.escapeHtml(fromName);
          const safeFromAvatar = this.escapeHtml(fromAvatar);
          const safeQuestion = this.escapeHtml(question);
          const safeAnswer = this.escapeHtml(answer);
          return BossOption.template
            .replace(/{{user_name}}/g, safeFromName)
            .replace(/{{user_avatar}}/g, safeFromAvatar)
            .replace(/{{user_question}}/g, safeQuestion)
            .replace(/{{assistant_name}}/g, "AI助手")
            .replace(/{{assistant_answer}}/g, safeAnswer)
        ...
        static buildJobKey(bossUserInfo) {
          ...
          return bossUserInfo.encryptJobId + ":" + Tools.getPageUidString() + "@" + channelKey;
        }
...
if (Tools.isBossDomainHost(Tools.getCurrentHostname())) {
  BossOption.loadRecentContact().then((_2) => {});
}
