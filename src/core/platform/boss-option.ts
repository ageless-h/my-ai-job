...
`;
        ...
        renderChatBubble(fromName, fromAvatar, question, answer) {
          return BossOption.template.replace(/{{user_name}}/g, fromName).replace(/{{user_avatar}}/g, fromAvatar).replace(/{{user_question}}/g, question).replace(/{{assistant_name}}/g, "AI助手").replace(/{{assistant_answer}}/g, answer).replace(/{{assistant_avatar}}/g, BossOption.aiIconWebpBase64);
        }
        async handleBossMessage(msgObj, bossId, text) {
          ...
          return AiPower.ask(text, jobKey, bossUserInfo).then((resp) => {
            ...
            if ((error == null ? void 0 : error.code) === BizCodeEnum.PRODUCT_NOT_AUTHORIZED || message.includes("不存在AI坐席且试用结束")) {
              logger$1.info("AI坐席付费限制已取消，忽略该返回", message);
              return Promise.resolve();
            }
            logger$1.warn("AI坐席处理消息失败", message);
            return Promise.resolve();
          });
        }
        ...
        static buildJobKey(bossUserInfo) {
          ...
          return bossUserInfo.encryptJobId + ":" + Tools.window._PAGE.uid + "@" + channelKey;
        }
